...
export const EMMI_TOOL_DECLARATIONS = [{ functionDeclarations: [
  ...
  { name: "startRefillReview", description: "Open the refill review only when the patient explicitly asks for a refill or clearly says they are running out. medicationId must be an exact ID returned by getMedicationSupply in this same turn; never pass raw patient speech, a drug name, pronoun, or phrase such as 'this program'. This starts a review; it never requests, approves or renews anything.", parameters: { type: "OBJECT", properties: { patientId: { type: "STRING" }, medicationId: { type: "STRING" } }, required: ["patientId", "medicationId"] } },
