---
id: enrollment-enrollment
title: Enrollment
category: enrollment
audience: patient
risk_level: medium
requires_patient_context: true
requires_tool_when_personalized: false
version: 1.0
last_reviewed: 2026-08-27
owner: ITERA HEALTH
keywords: inscripcion, inscripción, inscribirme, inscrito, enroll, enrolled, enrollment, enskri, enskripsyon, apuntarme, registrarme, voluntario
---

# Enrollment

Enrollment should be voluntary, understandable and distinct from later activation/getting-started steps.
Never imply a patient is enrolled until the trusted enrollment status says completed.
At natural stopping points, explain the next flow and let the patient choose whether to continue now or later.

## EMMI response rule

Use plain, senior-friendly language. If the question becomes patient-specific, prefer trusted runtime data and approved tools over static knowledge.

## Patient answer (EN)

Enrolling means agreeing to join ACCESS with ITERA HEALTH, and it is your choice. You review the details first, including what you would be expected to pay, and nothing is final until you agree. Enrolling is a separate step from getting set up afterwards, so you can finish the setup steps whenever you are ready and stop partway if you need to. Your progress is saved, and you can pick it up later.

## Patient answer (ES)

Inscribirse es voluntario y usted revisa la información antes de decidir. Inscribirse y configurar su cuidado son dos cosas distintas: primero se completa la inscripción, y después vienen los pasos para poner en marcha su cuidado. Nadie le dirá que está inscrito hasta que su registro lo confirme.

## Patient answer (KR)

Enskri se volontè epi ou revize enfòmasyon an anvan ou deside. Enskri ak konfigire swen ou se de bagay diferan: dabò enskripsyon an fini, apre sa vini etap yo pou mete swen ou an mach. Pesonn p ap di ou ke ou enskri jiskaske dosye ou konfime sa.
