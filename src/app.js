...
const identityFormValues = () => {
  const identity = invitationIdentity();
  return {
    dob: state.identityDraft?.dob ?? displayDate(identity.dobIso),
    zip: state.identityDraft?.zip ?? identity.zip
  };
};
...
let state = {
  ...
  accessNoticeAcknowledgedAt: "", disclosureAcknowledgedAt: "", disclosureVersion: "", accessDisclosureView: null, consentRole: "", consentVersion: "", consentTimestamp: "", consentAcknowledgement: null, sessionId: globalThis.crypto?.randomUUID?.() || `session_${Date.now().toString(36)}`, sessionMetadata: { platform: navigator.userAgentData?.platform || navigator.platform || "unknown" }, ipMetadata: null, identityVerified: false, identityDraft: null,
...
function identity() {
  ...
  const identityValues = identityFormValues();
  ...
  return `<h1 tabindex="-1">${representative ? L("Let’s securely confirm the patient’s identity", "Confirmemos de forma segura la identidad del paciente", "Ann konfime idantite pasyan an an sekirite") : L("Let’s securely confirm it’s you", "Confirmemos su identidad de forma segura", "Ann konfime se ou an sekirite")}</h1>
    ...
    <form id="identity-form" novalidate>
      <div class="field"><label for="dob">${L("Date of birth", "Fecha de nacimiento", "Dat nesans")}</label><div class="date-control"><input id="dob" class="date-text" name="dob" type="text" inputmode="numeric" autocomplete="bday" maxlength="14" value="${escapeHtml(identityValues.dob)}" placeholder="MM / DD / YYYY" aria-describedby="identity-helper identity-error"><input class="date-picker-native" type="date" min="1900-01-01" max="${localToday()}" value="${invitationIdentity().dobIso}" aria-label="${L("Choose date of birth from calendar", "Elegir fecha de nacimiento del calendario", "Chwazi dat nesans nan kalandriye a")}">${icon("calendar", "date-picker-icon")}</div><small class="field-helper">${L("Use MM / DD / YYYY.", "Use MM / DD / AAAA.", "Itilize MM / JJ / AAAA.")}</small></div>
      <div class="field"><label for="zip">${L("ZIP code", "Código postal", "Kòd postal")}</label><input id="zip" class="zip-input" name="zip" type="text" inputmode="numeric" pattern="[0-9]{5}" autocomplete="postal-code" maxlength="5" value="${escapeHtml(identityValues.zip)}" placeholder="${L("5-digit ZIP code", "Código postal de 5 dígitos", "Kòd postal 5 chif")}" aria-describedby="identity-helper identity-error"><small class="field-helper">${L("Enter your home ZIP code.", "Ingrese el código postal de su domicilio.", "Antre kòd postal lakay ou.")}</small></div>
...
function accessBpShippingAddress() {
  ...
  // A different address must be entered deliberately. Reusing the address on file here lets
  // browser autofill restore it after the patient clears a field, which can turn an incomplete
  // alternate address into an apparently valid request.
  const current = state.shippingAddress || (selected === "existing" ? existing : {}) || {};
  return `${art("box")}${titleBlock(L("Where would you like your monitor delivered?", "¿Dónde desea recibir su monitor?", "Ki kote ou vle resevwa aparèy ou a?"))}<form id="bp-shipping-form" novalidate><div class="choice-list">${choice("existing", "home", L("This address is correct", "Esta dirección es correcta", "Adrès sa a kòrèk"), L("Use the address we already have", "Use la dirección que ya tenemos", "Itilize adrès nou deja genyen an"), selected === "existing")}${choice("other", "document", L("Use a different address", "Usar otra dirección", "Itilize yon lòt adrès"), L("Enter another delivery address", "Ingrese otra dirección de envío", "Antre yon lòt adrès livrezon"), selected === "other")}</div>${selected === "existing" ? `<address class="address-card">${icon("home")}<span><strong>${L("Send to:", "Enviar a:", "Voye nan:")}</strong><br>${escapeHtml(existing.line1)}${existing.unit ? `<br>${escapeHtml(existing.unit)}` : ""}<br>${escapeHtml(existing.city)}, ${escapeHtml(existing.state)} ${escapeHtml(existing.zip)}</span></address>` : `<div class="shipping-fields"><div class="field"><label for="shipping-line1">${L("Street address", "Dirección", "Adrès lari")}</label><input id="shipping-line1" name="line1" autocomplete="shipping street-address" value="${escapeHtml(current.line1 || "")}" required aria-describedby="shipping-error"></div><div class="field"><label for="shipping-unit">${L("Apartment / Unit", "Apartamento / Unidad", "Apatman / Inite")}</label><input id="shipping-unit" name="unit" autocomplete="shipping address-line2" value="${escapeHtml(current.unit || "")}"></div><div class="field"><label for="shipping-city">${L("City", "Ciudad", "Vil")}</label><input id="shipping-city" name="city" autocomplete="shipping address-level2" value="${escapeHtml(current.city || "")}" required aria-describedby="shipping-error"></div><div class="shipping-short-fields"><div class="field"><label for="shipping-state">${L("State", "Estado", "Eta")}</label><input id="shipping-state" name="state" maxlength="2" pattern="[A-Za-z]{2}" autocomplete="shipping address-level1" value="${escapeHtml(current.state || "")}" required aria-describedby="shipping-error"></div><div class="field"><label for="shipping-zip">${L("ZIP code", "Código postal", "Kòd postal")}</label><input id="shipping-zip" name="zip" inputmode="numeric" maxlength="5" pattern="[0-9]{5}" autocomplete="shipping postal-code" value="${escapeHtml(current.zip || "")}" required aria-describedby="shipping-error"></div></div></div>`}<p id="shipping-error" class="form-error" role="alert">${state.error || ""}</p>${actions(state.busy ? L("Requesting…", "Solicitando…", "N ap mande…") : L("Request my monitor", "Solicitar mi monitor", "Mande aparèy mwen an"), true, "", state.busy)}</form>`;
}
...
async function advance() {
  ...
  if (state.screen === "IDENTITY_VERIFICATION") {
    ...
    state.identityDraft = { dob: data.dob, zip: String(data.zip || "") };
    ...
    state.identityVerified = true; state.identityDraft = null; audit(state, "identity_verified");
...
const languageDeclineCopy = locale => ({
  es: "No hay problema — continuaré en español.",
  ht: "Pa gen pwoblèm — m ap kontinye an kreyòl.",
  en: "No problem — I’ll continue in English."
})[locale];
...
function handlePatientLanguage(text) {
  ...
  if (offered && offered !== activeLocale) {
    ...
    if (isLanguageOfferDeclined(text)) {
      ...
      state.emmiPendingLanguageQuestion = "";
      const response = languageDeclineCopy(activeLocale);
      state.assistantMessages.push({ role: "assistant", text: response, intent: "LANGUAGE_DECLINED" });
...
function bind() {
  bindActions = root => root.querySelectorAll("[data-action]").forEach(el => el.addEventListener("click", async event => {
    ...
    if (action === "add-medication") {
      ...
      const frequency = String(data.medicationFrequency || "");
      const item = { id: state.medicationEditId || `patient-med-${Date.now().toString(36)}`, medicationName: name.slice(0, 80), dose: String(data.medicationDetails || "").trim().slice(0, 120), frequency, frequencyLabel: frequency ? form.querySelector('[name="medicationFrequency"] option:checked')?.textContent || "" : "", source: "PATIENT_REPORTED", createdAt };
