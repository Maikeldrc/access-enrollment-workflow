...
// how a chest-pain sentence ends up answered from the knowledge base. Sudden one-sided weakness,
// facial droop and slurred speech are here with chest pain: they are the FAST stroke signs, and the
// whole value of recognising a stroke is recognising it in the first minutes.
export const EMERGENCY_SYMPTOM_PATTERN = /chest pain|can.?t breathe|cannot breathe|difficulty breathing|short(?:ness)? of breath|stroke|severe bleeding|very bad|pass(?:ed)? out|faint(?:ed|ing)?|suicid|emergency|weak(?:ness)?\s+(?:on|in|down)\s+(?:one|the left|the right|my left|my right|left|right)\s+side|numb(?:ness)?\s+(?:on|in)\s+(?:one|the left|the right|my left|my right|left|right)\s+side|(?:one|left|right) side of (?:my |the )?(?:body|face)|(?:face|mouth)\s+(?:is\s+)?droop|facial droop|slurred|slurring|trouble (?:speaking|talking)|difficulty speaking|can(?:'?t| ?not) speak (?:properly|clearly|right)|seizure|convulsion|unconscious|unresponsive|won'?t wake up|crushing (?:chest )?pain|pressure in my chest|(?:one|left|right) side[^.?!]{0,25}\b(?:weak|numb|paralys|droop|gone dead)|worst headache|dolor (?:muy )?(?:fuerte |agudo |intenso )?(?:en (?:el|la) )?pecho|dolor de pecho|no puedo respirar|dificultad para respirar|muy mal|derrame|sangrado grave|me desmay|desmayo|emergencia|debilidad (?:en|de) un lado|entumecimiento (?:en|de) un lado|un lado (?:del|de mi) cuerpo|se me (?:cae|torci[oó]) la cara|no puedo hablar|habla arrastrada|convulsi[oó]n|no responde|no despierta|peor dolor de cabeza|suicid|doulè nan pwatrin|pa ka respire|konjesyon serebral|senyen anpil|endispoze|pèdi konesans|ijans|swisid|febl[eè]s yon b[oò]|figi (?:l|mwen) tonbe|pa ka pale|lang mwen mare|kriz|pa reveye|\bmy (?:chest|heart|arm|jaw)\b[^.?!]{0,15}\b(?:hurts?|hurting|aching|killing me)\b|\bpain in my (?:chest|heart|arm|jaw)\b|\bme duele[^.?!]{0,12}(?:pecho|coraz[oó]n|brazo|mand[ií]bula)|\b(?:pwatrin|k[oè]|bra) (?:mwen |m )?f[eè] (?:m |mwen )?mal\b/i;
