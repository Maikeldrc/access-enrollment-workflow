...
export function journeyFor(s) {
  ...
  if (p === "RPM") return [...START, "DISCLOSURE", "CONSENT_REVIEW", "ENROLLMENT_PROCESSING", "ENROLLMENT_CONFIRMED", "RPM_DEVICE_PATH", ...(s.devicePath === "ship" ? ["RPM_ADDRESS_CONFIRMATION"] : []), "RPM_DEVICE_SETUP", "RPM_FIRST_READING", "RPM_MONITORING_READY"];
...
export function progressFor(s) {
  ...
  return isSetup ? { label: s.offer.pathway === "RPM" ? "Home monitoring setup" : "Care setup", current: index - setupStart + 1, total: j.length - setupStart } : { label: "Enrollment", current: index + 1, total: setupStart > -1 ? setupStart : j.length };
}
