...
export const buildEmmiLiveTokenConfig = ({ model, expireTime, newSessionExpireTime }) => ({
  uses: 1,
  expireTime,
...
export async function handleEmmiLiveToken(req, res, env = process.env) {
  ...
  try {
    ...
    return json(res, 200, { token: token.name, model, expiresAt: expireTime, prototype: true });
  } catch (error) {
