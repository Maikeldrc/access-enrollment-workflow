...
const P = {
  ...
  afterConfirm: `const v = view || {}; const done = (v.alreadyDone || []).join(" "); const pend = (v.stillPending || []).join(" "); if (/reservado|booked|cambiada|enviada|moved|sent/i.test(done)) return "Listo. El viaje quedó reservado."; if (/fall[óo]|failed|no se reserv/i.test(pend + " " + (v.whatThePatientMustDoHere || ""))) return "No se pudo reservar: el proveedor no estaba disponible. No se cobró nada. Puedo intentar de nuevo o buscar otro vehículo."; return \`Estoy reservando el viaje. Todavía no está confirmado. \${(v.stillPending || [])[0] || ""}\`;`,
  ...
  isBooked: `const v = view || {}; const done = (v.alreadyDone || []).join(" "); const pend = (v.stillPending || []).join(" "); if (/reservado|booked/i.test(done)) return "Sí, el viaje ya quedó reservado."; if (/fall[óo]|failed|no se reserv/i.test(pend + " " + (v.whatThePatientMustDoHere || ""))) return "No. La reserva falló y no se cobró nada. ¿Intento de nuevo o busco otro vehículo?"; return \`Todavía no. \${(v.stillPending || [])[0] || v.whatThePatientMustDoHere || ""}\`;`,
...
export const SCENARIOS = [
  ...
  },
  {
    ...
    steps: [
      ...
      { speak: { text: "Mejor no quiero cambiarla.", model: act({ actionId: "barrier-back" }, `const r = results[0] || {}; const v = view || {}; if (r.success === false) return "No puedo salir de esta pantalla por usted; su cita no ha cambiado. Toque 'Elegir otro horario' o vuelva a la cita."; return \`De acuerdo, no cambio nada. Su cita sigue como estaba: \${(v.onScreen || []).map(f => Object.values(f)[0]).slice(1,3).join(", ")}.\`;`), notes: "retraction on the review screen: nothing may be confirmed, and EMMI must be able to step back" } },
  ...
  },
  {
    ...
    steps: [
      ...
      { speak: { text: "Can you pick it for me?", model: act({ optionRef: "1" }, `const r = results[0] || {}; const v = view || {}; if (r.success === false || v.youMayPressTheseYourself === false) return "I can't press that one for you on this screen. Tap 'For myself', then Continue."; return \`Done: \${v.selected?.label || "selected"}.\`;`), notes: "the enrollment screens are the patient's to press; the tool must refuse and EMMI must say so" } },
