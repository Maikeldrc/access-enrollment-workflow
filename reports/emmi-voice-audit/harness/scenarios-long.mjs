...
export const LONG_SCENARIOS = [
  ...
  },
  {
    ...
    steps: [
      ...
      { speak: { text: "Hmm. Actually, cancel it.", model: chain([pv("barrier-return-no"), pv("barrier-ride-cancel")], E.cancelAsk), notes: "the app is still asking about the ride home; a compliant model answers that first (no return ride), then opens the cancel confirmation — destructive: must ask before cancelling" } },
      ...
      { speak: { text: "Wait, no! I do need it after all.", model: chain([pv("barrier-accept"), pv("barrier-pickup-home"), opt(1), pv("barrier-needs-continue"), pv("barrier-time-accept")], `return "No problem. Searching again.";`), notes: "change of mind again: from the cancelled state the screen offers 'Arrange another ride'" } },
