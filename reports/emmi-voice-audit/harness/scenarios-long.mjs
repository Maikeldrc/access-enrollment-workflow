...
const E = {
  ...
  afterConfirm: `const v = view || {}; const done = (v.alreadyDone || []).join(" "); if (/booked|reservado/i.test(done)) return "Done. Your ride is booked."; return \`I'm booking it now. It is not confirmed yet. \${(v.stillPending || [])[0] || ""}\`;`,
  isBooked: `const v = view || {}; const done = (v.alreadyDone || []).join(" "); return /booked|reservado/i.test(done) ? "Yes, the ride is booked." : \`Not yet. \${(v.stillPending || [])[0] || v.whatThePatientMustDoHere || ""}\`;`,
  ...
  cancelled: `const v = view || {}; const done = (v.alreadyDone || []).join(" "); return /cancel/i.test(done + " " + (v.viewId || "")) ? "The ride is cancelled." : \`\${v.whatThePatientMustDoHere || "The ride was not cancelled."}\`;`,
...
export const LONG_SCENARIOS = [
  ...
  },
  {
    ...
    notes: "19 spoken turns in English. 'Actually cancel it' → 'Wait, no!' → rebook. Includes a long reply interrupted with 'Stop.'",
    steps: [
      { navigate: { selector: '[data-action="appointment-open"]', label: "open the appointment" } },
      ...
      { speak: { text: "The cheapest one.", model: describe(E.cheapestPick), expect: { viewId: "REVIEW" } } },
      ...
      { speak: { text: "Hmm. Actually, cancel it.", model: act({ actionId: "barrier-ride-cancel" }, E.cancelAsk), notes: "destructive: must ask before cancelling" } },
      { speak: { text: "Yes, cancel it.", model: act({ actionId: "barrier-ride-cancel-confirm", confirmed: true }, E.cancelled), action_requested: "cancel ride" } },
      { speak: { text: "Wait, no! I do need it after all.", model: chain([pv("appointment-open-barrier"), opt(2), pv("barrier-accept"), pv("barrier-pickup-home"), opt(1), pv("barrier-needs-continue"), pv("barrier-time-accept")], `return "No problem. Searching again.";`), notes: "change of mind again" } },
      ...
      { speak: { text: "Just book the one with more room.", model: describe(E.roomiestPick), expect: { viewId: "REVIEW" } } },
      ...
      { speak: { text: "What time is the pickup?", model: describe(field("Hora de recogida|Pickup time", "`Pickup is at ${Object.values(f)[0]}.` : \"I don't see a pickup time.\"")) } },
  ...
  },
  {
    ...
    seed: appt => ({ language: HT, appointments: [appt()] }),
    notes: "Short sample only: what the app does with a Kreyòl session (narration language, view labels, language switch on request). The double transcribes the Kreyòl text as given.",
    steps: [
      { navigate: { selector: '[data-action="appointment-open"]', label: "open the appointment" } },
      { speak: { text: "Bonjou.", model: { text: "Bonjou. Mwen la pou ede w ak randevou w la." } } },
      { speak: { text: "Ki lè randevou mwen an?", model: describe(field("Lè|Hora|Time", "`Li a ${Object.values(f)[0]}.` : \"Mwen pa wè lè a.\"")), expect: { contextBeforeAnswer: true } } },
      { speak: { text: "Mwen pa gen transpò.", model: chain([pv("appointment-open-barrier"), opt(2), pv("barrier-accept")], `const home = (view?.choices || [])[0]; return home ? \`Nou ka chèche transpò. Nou pran w lakay ou, \${home.detail}?\` : "Ki kote pou nou pran w?";`), expect: { viewId: "PICKUP" } } },
      { speak: { text: "Wi.", model: act({ actionId: "barrier-pickup-home" }, `return "Èske w bezwen èd espesyal pou vwayaje?";`) } },
      { speak: { text: "Mejor hábleme en español, por favor.", model: { text: "Dakò." }, timeoutMs: 30000, notes: "explicit language request inside a Kreyòl session" } },
      { silence: { ms: 4000, label: "session rebuilt" } },
      { custom: async ({ page, recorder }) => { const lang = await page.evaluate(() => document.documentElement.lang); const sessions = await page.evaluate(() => window.__fakeLive.sessionCount); recorder.observe(`after the request: html lang=${lang}; provider sessions opened=${sessions}`); recorder.session.language_switch = { lang, sessions }; } },
      { speak: { text: "¿Necesito decir algo más?", model: describe(P.whatHere) } },
      { speak: { text: "Gracias.", model: { text: "De nada." } } }
    ]
  },
  {
